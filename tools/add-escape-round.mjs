#!/usr/bin/env node
/* =========================================================================
   add-escape-round.mjs: append a room to an escape game, spliced in at the
   tail so the chain grows  home → r1 → r2 → … → finish.

   Interactive:   node tools/add-escape-round.mjs
   From a file:   node tools/add-escape-round.mjs --json path/to/room.json

   What it does
     1. finds the escape (its escape.config.json) — auto-selects if there's one.
     2. asks how many ciphers this room has, then collects each cipher just like
        add:question. Unlike the relay there is NO "solve N to advance" — a team
        can crack the puzzle and type the password whenever they figure it out.
     3. writes the room page at  <base>/roomN-<hex>/index.html  with the ciphers,
        the submission form (open-in-new-tab + optional in-page embed), and an
        ALWAYS-visible password gate. The unpredictable slug keeps the URL from
        being guessed / jumped to.
     4. re-points the current tail (the home page, or the previous room) at this
        new room, and points this room at the finish page.

   Passwords are never stored. Each page ships its "next" link ENCRYPTED
   (gate.enc) under the password a team works out from the puzzle (the same one
   the room's Google Form reveals on its confirmation message). To re-point the
   old tail we need its password, so the tool asks for it and verifies it before
...
const TPL_PATH = join(ROOT, "tools/templates/escape-round.html");
...
const DEFAULT_LEAD = "Crack the ciphers to work out the room's password, submit your answers, then type the password to open the door.";
...
// or verifying, exactly as escape.js does before decrypting.
...
const CFG_RE = /(<script id="escape-config" type="application\/json">)([\s\S]*?)(<\/script>)/;
...
/** Walk the repo for escape.config.json files (skips node_modules, .git, dotfiles). */
async function findEscapes(dir = ROOT, out = []) {
  let entries;
  try { entries = await readdir(dir, { withFileTypes: true }); } catch { return out; }
  for (const e of entries) {
    if (e.name.startsWith(".") || e.name === "node_modules") continue;
    const full = join(dir, e.name);
    if (e.isDirectory()) await findEscapes(full, out);
    else if (e.name === "escape.config.json") {
...
async function collectCipher(ask, idx, total) {
  ...
  data.revealKeyword = await ask("  Keyword revealed when it's solved (a clue toward the password)");
...
async function collectInteractive(rl, configs) {
  ...
  let chosen = await pickEscape(configs);
  if (!chosen) {
    console.log("\n  Which escape?");
  ...
  console.log(`\n  Escape: ${config.title} — adding Room ${nextNum}`);

  const count = Math.max(1, parseInt(await ask("\n  How many ciphers in this room?", "4"), 10) || 4);

  const questions = [];
  for (let i = 0; i < count; i++) questions.push(await collectCipher(ask, i, count));

  console.log("\n  Submission form (shown on the room page, always visible):");
  const formUrl = await askRequired("    This room's Google Form URL (the full …/viewform link)");
  const formEmbed = /^y/i.test(await ask("    Embed the form in the page too? (y = embed + new-tab button, n = new-tab button only)", "y"));
  const code = await askRequired("    Password to the NEXT room (what a team types in THIS room's box; shown on THIS form's confirmation screen)");

  const isFirst = !config.rounds || config.rounds.length === 0;
  console.log("\n  Link the previous page to this room:");
  const prevLabel = isFirst
    ? "    Team password from the registration form (points the start page at this room)"
    : `    Password for Room ${config.rounds.length} (points it at this room)`;
  const prevCode = await askRequired(prevLabel);

  return { chosen, config, count, questions, formUrl, formEmbed, code, prevCode, lead: DEFAULT_LEAD };
}

async function fromJson(p, configs) {
  const j = JSON.parse(await readFile(p, "utf8"));
  const chosen = await pickEscape(configs, j.escape);
  if (!chosen) throw new Error('Multiple escapes found — set "escape": "<base>" in the JSON to pick one.');
  if (!Array.isArray(j.questions) || !j.questions.length) throw new Error("room JSON needs a non-empty questions array.");
  if (!j.formUrl) throw new Error("room JSON needs formUrl.");
  if (!j.code) throw new Error("room JSON needs code (the password to the next room).");
  if (!j.prevCode) throw new Error("room JSON needs prevCode (the current tail's password).");
...
async function main() {
  ...
  const configs = await findEscapes();
  if (!configs.length) throw new Error("No escape found — run `npm run add:escape` first.");
  ...
  const tailKind = isFirst ? "start page" : `Room ${config.rounds.length}`;

  // ---- verify the previous password BEFORE writing anything (don't corrupt the chain) ----
  ...
  if (!tailMatch) throw new Error(`No escape-config found in the ${tailKind}.`);
  const tailCfg = JSON.parse(tailMatch[2]);
  const currentTarget = tailCfg.gate && tailCfg.gate.enc ? dec(tailCfg.gate.enc, normPw(f.prevCode)) : null;
  if (currentTarget !== config.finishPath) {
    throw new Error(
      `That password doesn't unlock the current ${tailKind}. Right now its gate should decrypt to the finish page` +
      ` (${config.finishPath}); with the password you gave it decrypts to ${currentTarget === null ? "nothing" : `"${currentTarget}"`}.` +
      `\n  Re-check the password you set on ${isFirst ? "the registration form" : `Room ${config.rounds.length}'s form`}.`,
    );
  }

  // ---- build this room ----
  const slug = `room${num}-${randomBytes(3).toString("hex")}`;
  ...
  const roundCfg = {
    ...
    gate: { enc: enc(config.finishPath, normPw(f.code)) },
  };

  const formSrc = formEmbedSrc(f.formUrl);
  const formEmbedHtml = f.formEmbed
    ? `          <div class="embed relay-form">\n` +
      `            <iframe src="${esc(formSrc)}" title="Room ${num} submission form" loading="lazy">Loading…</iframe>\n` +
      `          </div>`
    : "";

  const crumbs = `<a href="/">Home</a> / <a href="${config.homePath}">${esc(config.title)}</a> / Room ${num}`;
  const html = fill(await readFile(TPL_PATH, "utf8"), {
    TITLE: `Room ${num}`, DESC: esc(`Room ${num} of ${config.title}.`), CRUMBS: crumbs,
    HEADING: `Room ${num}`, LEAD: esc(f.lead),
  ...
  // ---- re-point the old tail at this room ----
  tailCfg.gate.enc = enc(roundHref, normPw(f.prevCode));
  ...
  // ---- record structure (no passwords) ----
  config.rounds.push({ num, slug, path: roundHref, formUrl: f.formUrl });
  await writeFile(chosen.path, JSON.stringify(config, null, 2) + "\n", "utf8");

  // ---- report ----
  const chain = ["home", ...config.rounds.map((r) => "r" + r.num), "finish"].join(" → ");
  console.log(`\n  ✓ Created  ${relative(ROOT, roundAbs)}  (Room ${num}: ${f.count} ciphers)`);
  if (imgCount) console.log(`  ✓ Images   copied ${imgCount} into ${CRYPT_IMG_DIR}/`);
  console.log(`  ✓ Linked   ${tailKind} → Room ${num} → finish`);
  console.log(`  ✓ Updated  ${relative(ROOT, chosen.path)}`);
  console.log(`\n  Chain:     ${chain}`);
  console.log(`\n  On Room ${num}'s Google Form:`);
  console.log(`   • set the confirmation message to reveal the password:  ${normPw(f.code)}  (case-insensitive)`);
  console.log(`   • the form and the password box stay open the whole time (no "solve N to advance").`);
